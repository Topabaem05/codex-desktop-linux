...
本版为明确配置的本地 stdio MCP 加入原生生命周期管理器。由后端直接启动的
轻量 C 启动器和隔离的 C 监督器组成；直接启动器被 KILL 时，监督器仍能清理
其拥有的命令进程组。保留参数、环境、工作目录和输出，输入使用固定 64 KiB 缓冲。
...
历史未包装的进程。主动脱离进程组、隔离监督器本身被 KILL、其他构建工具或
...
该选项仅跳过 Community 包装，不能改变上游对其子进程的生命周期控制。
